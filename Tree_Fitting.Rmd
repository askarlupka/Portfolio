...
d <- d %>%
  select(c(Action1, CasesAll, Country, Deaths, EndMonth, gg2c4, Hemisphere, Hospitalizations, MeanD1, MeanI1, MedianD1, MedianI1, OBYear, Path1, RateAll, RiskAll, season, Setting_1, StartMonth, State, Trans1, Vomit))
```
...
unique(d$Hemisphere)
d$Hemisphere <- as.factor(d$Hemisphere)
levels(d$Hemisphere)
...
d$Path1 <- as.factor(d$Path1)
levels(d$Path1)
d$Path1 <- fct_collapse(d$Path1, Unspecified = c("Unknown", "Unspecified"), Yes = "Yes", No = "No")
levels(d$Path1)

#Group setting_1 into only two factors
...
#lump together the countries
d$Country <- as.factor(d$Country)
levels(d$Country)
d$Country <- fct_collapse(d$Country, Other = c("Australia", "Ca da", "Chi", "Croatia", "Denmark", "France", "Iraq", "Israel", "Italy", "Multiple", "New Zealand", "Norway", "Other", "Scotland", "Spain", "UK", "Unspecified"), USA = "USA", Japan = "Japan")
...
levels(d$Trans1)
d$Trans1 <- fct_collapse(d$Trans1, Environmental = c("Environmental", "Waterborne"), Unspecified = c("Unknown", "Unspecified"))
levels(d$Trans1)

#drop the state variable because there are 841 observations of zero.
table(d$State)
d <- d %>%
  select(-State)
...
numeric <- d %>%
  select(c(CasesAll, Deaths, Hospitalizations, MeanD1, MeanI1, MedianD1, MedianI1, RiskAll, RateAll, OBYear, StartMonth, EndMonth))
visdat::vis_cor(numeric)
```
...
trainIndex <- createDataPartition(d$season, p = .7, 
                                  list = FALSE, 
                                  times = 1)
...
length(data_train$season) #there are a total of 668 observations
null <- 309/668
...
print(fit1_clean$results)

prp(fit1_clean$finalModel, extra = 1, type = 1)
ww=17.8/2.54; wh=ww; #for saving plot
dev.print(device=png,width=ww,height=wh,units="in",res=600,file="rparttree.png") #save tree to file
...
*I'm not really seeing a difference between my random forests*
...
confusionMatrix(data = data_train$prediction, reference = data_train$season)
...
confusionMatrix(data = data_test$prediction, reference = data_test$season)
